...
public:
    ...
    ColliderInfoCapsule(bool is_trigger, uint32_t category = Category::kAll,
                        uint32_t mask = Category::kAll);

    /// @brief カプセルパラメータ付きコンストラクタ
    ColliderInfoCapsule(const VECTOR& local_start, const VECTOR& local_end,
