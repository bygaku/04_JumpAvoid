...
{
    // 線分の両端を計算
    VECTOR world_start = VAdd(position, world_start_);
    VECTOR world_end   = VAdd(position, world_end_);
    ...
    // 少し膨らませる（点での判定を避けるため）
    VECTOR margin = VGet(0.1f, 0.1f, 0.1f);
    aabb_.SetMin(VSub(min_point, margin));
    aabb_.SetMax(VAdd(max_point, margin));
    needs_update_ = false;
}
